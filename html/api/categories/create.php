...
error_log("Create category request - ID: {$categoryId}, Name: {$name}, Color: {$color}");

// Safety check: if category_id is set, this should be an update, not a create
if ($categoryId > 0) {
    http_response_code(400);
    echo renderError('Error: Category ID should not be set for create operation. Use update endpoint instead.');
    exit;
}
...
// Create category
$result = createCategory($userId, $name, $color);
...
if ($result['success']) {
    ...
    http_response_code(400);
    echo renderError($result['error'] ?? 'Failed to create category');
}
